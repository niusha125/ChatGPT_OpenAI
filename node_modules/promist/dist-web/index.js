...
function intercept(promise, interceptFn) {
  ...
  promise.then = function () {
    return run().then(...arguments);
  };

  promise.catch = function () {
    return run().catch(...arguments);
  };

  if (promise.hasOwnProperty('finally')) {
    promise.finally = function () {
      return run().finally(...arguments);
    };
  }
...
function waitUntil(testCb) {
  let ms = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 20;
...
function delay(ms) {
  let delayRejection = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  const init = Date.now();
...
function trunk() {
  ...
  return fns.reduce((a, b) => function () {
  ...
  return (
    ...
    _asyncToGenerator(function* () {
      ...
      while (!done && (testRes = yield test()) && !(testRes instanceof Error)) {
        const next = iterator.next((yield value));
        value = next.value;
        done = next.done;
      }
...
export { cancellable, compose, control, deferrable, deferred, delay, immediate, isPromise, lazy, parallel, series, status, timed, timeout, wait, waitUntil };
